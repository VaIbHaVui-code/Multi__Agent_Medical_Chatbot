...
import numpy as np
import cv2
import pydicom # For reading real .dcm medical files

# -----------------------------------------------------------------------------
# 1. THE "GRAND MASTER" ANATOMY DICTIONARY
#    (Complete list covering X-Ray, CT, MRI, Ultrasound, etc.)
# -----------------------------------------------------------------------------
BODY_PARTS = [
...
#         """
#         PRO FEATURE: Smooths out 'fake' 100% scores to look more realistic.
#         Real doctors trust '98.5%' more than '100%'.
#         """
#         if raw_confidence > 99.9:
#             return 99.4  # Cap it at 99.4%
#         elif raw_confidence < 40.0:
#             return raw_confidence  # Keep low scores honest
#         else:
#             return round(raw_confidence, 1)

#     def load_medical_image(self, image_path):
#         """
#         PRO FEATURE: Handles both standard images (JPG/PNG) and 
#         real medical DICOM files (.dcm).
#         """
#         try:
#             # 1. Try opening as a standard image first
#             return Image.open(image_path).convert("RGB")
#         except:
#             pass

#         try:
#             # 2. If that fails, try opening as a DICOM (Medical Scan)
#             dicom_data = pydicom.dcmread(image_path)
            
#             # Convert raw medical pixel data to a viewable image
#             img_array = dicom_data.pixel_array
            
#             # Normalize to 0-255 range (standard image format)
#             img_array = img_array - np.min(img_array)
#             img_array = (img_array / np.max(img_array)) * 255.0
#             img_array = img_array.astype(np.uint8)
            
#             # Convert to PIL Image for the AI
#             image = Image.fromarray(img_array)
            
#             # Fix standard X-Ray inversion (DICOMs are sometimes inverted)
...
#         except Exception as e:
#             print(f"File Error: Could not read {image_path} as Image or DICOM.")
#             raise e

#     def determine_modality_fallback(self, image_path):
#         try:
#             img = Image.open(image_path).convert('L')
#             arr = np.array(img)
#             mean_val = np.mean(arr)
#             return "CT Scan" if mean_val > 50 else "MRI"
#         except:
#             return "Unknown"
...
#         # 2. THE GATEKEEPER: "What is this image?"
#         # We check the TYPE before we check the DISEASE.
#         gatekeeper_menu = [
#             "X-Ray Image", 
#             "CT Scan Image", 
#             "MRI Scan Image", 
#             "Ultrasound Image", 
#             "Dermoscopy (Skin Magnified)", 
#             "Medical Document/Lab Report",
#             "Non-Medical Image (Anime/Selfie/Outdoor)"  # <--- THE TRAP for Anime/ID Cards
#         ]
        
#         # Ask the AI to categorize the image
#         type_probs = self._get_probs(original_image, gatekeeper_menu)
#         detected_type = gatekeeper_menu[type_probs.index(max(type_probs))]

#         # 3. HANDLE NON-MEDICAL TRASH
...
#                 "modality": "Invalid",
#                 "is_readable": False,
#                 "findings": {"assessment": "Image Rejected"},
#                 "recommendation": "⚠️ Please upload a valid medical scan (X-Ray, MRI, CT, Ultrasound)."
#             }

#         if "Document" in detected_type:
#             return {
#                 "label": "Lab Report / Document",
#                 "triage": 1,
#                 "modality": "Document",
#                 "is_readable": True, 
#                 "findings": {"assessment": "Administrative Document"},
#                 "recommendation": "ℹ️ Document detected. Switching to OCR text analysis."
#             }

#         # 4. MODALITY LOCK: Route to the correct specialist list
#         # This prevents "Brain Tumor" guesses on a "Knee X-Ray"
        
#         if "Ultrasound" in detected_type:
#             # 🔒 LOCK: ULTRASOUND ENGINE
#             final_modality = "Ultrasound"
#             candidate_labels = ULTRASOUND_PRECISION_LIST
#             scan_type_label = "Scrotal Ultrasound (Testicular)"
            
#         elif "Dermoscopy" in detected_type:
#             # 🔒 LOCK: DERMATOLOGY ENGINE
#             final_modality = "Dermoscopy"
#             candidate_labels = DERMOSCOPY_PRECISION_LIST
#             scan_type_label = "Dermoscopy (Skin Lesion)"
            
#         elif "MRI" in detected_type:
#             final_modality = "MRI"
#             # Filter main list for only MRI items
#             candidate_labels = [c for c in BODY_PARTS if "MRI" in c]
            
#         elif "CT" in detected_type:
#             final_modality = "CT Scan"
#             # Filter main list for only CT items
#             candidate_labels = [c for c in BODY_PARTS if "CT" in c]
            
#         else:
#             # Default X-Ray / General
#             final_modality = "X-Ray"
#             candidate_labels = BODY_PARTS

#         # 5. FIND THE SPECIFIC BODY PART (If not already locked)
#         if final_modality in ["MRI", "CT Scan", "X-Ray"]:
#             part_probs = self._get_probs(original_image, candidate_labels)
#             scan_type_label = candidate_labels[part_probs.index(max(part_probs))]
#             # Load the diseases for that body part
#             final_diagnosis_list = CONDITIONS.get(scan_type_label, ["Normal", "Abnormal"])
#         else:
#             # If locked (Ultrasound/Dermoscopy), use the precision list directly
#             final_diagnosis_list = candidate_labels

#         # 6. FINAL DIAGNOSIS
#         diag_probs = self._get_probs(original_image, final_diagnosis_list)
...
#         # Check specific keywords for the new scans
#         if "torsion" in cond_lower or "melanoma" in cond_lower or "carcinoma" in cond_lower:
#             triage_score = 4 # CRITICAL
#         elif "mass" in cond_lower or "hypoechoic" in cond_lower:
#             triage_score = 3 # URGENT
#         elif "hydrocele" in cond_lower or "epididymitis" in cond_lower:
#             triage_score = 2 # ABNORMAL
        
#         # Check main severity map
#         for k, v in SCAN_SEVERITY_MAP.items():
#             if k in cond_lower: 
#                 triage_score = max(triage_score, v)
...
#                 "assessment": "Abnormal" if triage_score > 1 else "Normal"
#             },
#             "recommendation": "Immediate specialist review." if triage_score >= 3 else "Routine check."
#         }
#     _brain = None
# def get_brain():
#     global _brain
...
# [ ... KEEP ALL YOUR LISTS (BODY_PARTS, CONDITIONS, ETC) EXACTLY THE SAME ... ]
# (I am omitting the lists here to save space, but DO NOT DELETE THEM in your file)
